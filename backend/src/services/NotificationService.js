...
export class NotificationService {
  ...
  async processNotification(notification) {
    try {
      ...
      const userPreferences = await UserService.getUserPreferences(notification.userId);
      const results = [];
          ...
          switch (channel) {
            ...
            case 'email':
              if (userPreferences.preferences.notifications?.email !== false) {
                ...
                result = { success: false, reason: 'Email notifications disabled' };
              }
              break;
            case 'sms':
              if (userPreferences.preferences.notifications?.sms === true) {
                ...
                result = { success: false, reason: 'SMS notifications disabled' };
              }
              break;
            case 'push':
              if (userPreferences.preferences.notifications?.push !== false) {
                ...
                result = { success: false, reason: 'Push notifications disabled' };
              }
              break;
            default:
              result = { success: false, reason: `Unknown channel: ${channel}` };
          }
      ...
      notification.status = results.some(r => r.success) ? 'delivered' : 'failed';
  ...
  async sendPushNotification(user, notification) {
    try {
      ...
      return {
        success: sent,
        deliveredAt: sent ? new Date().toISOString() : null,
        reason: sent ? 'Push notification delivered' : 'User not connected for push'
      };
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            ${data.details ? `<div style="margin-top: 20px; padding: 15px; background: white; border-radius: 5px;"><pre style="white-space: pre-wrap;">${JSON.stringify(data.details, null, 2)}</pre></div>` : ''}
        </div>
        <div class="footer">
